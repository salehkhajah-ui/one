...
| Route | Surface |
| --- | --- |
| `/network` | Landing — the three-sided story + 60-second demo script |
...
The original allocation app is untouched at `/` — `lib/network/paths.ts`
routes the chrome (shell width, first-run gate, bottom nav) between the two
products.
...
`lib/engine/__tests__/network.test.ts` (22 tests) covers the master-spec
